...
#define GO_TO_SAME_FLOOR_REPEAT_COUNT 4
...
static elevator_state_t s_ElevatorState                 = kElevatorState_Idle;
...
{
    if (lv_obj_has_flag(guider_ui.home_img_mic_on, LV_OBJ_FLAG_HIDDEN))
    {
...
{
    if (lv_obj_has_flag(guider_ui.home_img_mic_off, LV_OBJ_FLAG_HIDDEN))
    {
...
static void _unmark_floor_buttons()
{
...
static void _arrive_floor_callback(lv_timer_t *timer)
{
    ...
    else
    {
        if (s_Arrived)
        {
            PlayPrompt(PROMPT_OPEN_DOOR_TONE, 1);
            ...
            s_ElevatorState = kElevatorState_Waiting;
        }
        else
        {
            if (s_ElevatorState == kElevatorState_StartSameFloor)
            {
                PlayPrompt(PROMPT_CONFIRM_TONE, 1);
                lv_obj_clear_state(guider_ui.home_imgbtn_doorOpen, LV_STATE_PRESSED);
                lv_obj_add_state(guider_ui.home_imgbtn_doorClose, LV_STATE_PRESSED);
                s_ElevatorState = kElevatorState_DoorClose;
            }
            else if (s_ElevatorState == kElevatorState_DoorClose)
            {
                PlayPrompt(PROMPT_CONFIRM_TONE, 1);
                lv_obj_clear_state(guider_ui.home_imgbtn_doorClose, LV_STATE_PRESSED);
                lv_obj_add_state(guider_ui.home_imgbtn_doorOpen, LV_STATE_PRESSED);
                s_ElevatorState = kElevatorState_DoorOpen;
            }
            else if (s_ElevatorState == kElevatorState_DoorOpen)
            {
                s_ElevatorState = kElevatorState_Arrival;
...
void go_to_floor(uint32_t floorNum)
{
    ...
    s_Arrived         = false;
    s_ElevatorState   = kElevatorState_Start;
    ...
    {
        s_ElevatorState = kElevatorState_StartSameFloor;
        PlayPrompt(PROMPT_CONFIRM_TONE, 1);
        lv_obj_add_state(guider_ui.home_imgbtn_doorOpen, LV_STATE_PRESSED);
    }
    ...
    timeToRepeat = (s_GoingToFloorNum == s_CurFloorNum) ? GO_TO_SAME_FLOOR_REPEAT_COUNT :
                                                          (abs(s_CurFloorNum - s_GoingToFloorNum) + 2);
    lv_timer_set_repeat_count(s_MotionTimer, timeToRepeat);
}
