...
void _HAL_InputDev_IrqHandler(button_data_t *button, switch_press_type_t pressType)
{
    if (s_InputDev_PushButtons.cap.callback != NULL)
    {
        ...
        uint32_t size         = 0;
        if (APP_InputDev_PushButtons_SetEvent(button->buttonId, pressType, &s_pEvent, &receiverList, &size) ==
            kStatus_Success)
        {
            /* Build input_event */
            s_inputEvent.eventId                  = kInputEventID_Recv;
            s_inputEvent.size                     = size;
            ...
            s_InputDev_PushButtons.cap.callback(&s_InputDev_PushButtons, &s_inputEvent, fromISR);
        }
        else
        {
...
hal_input_status_t HAL_InputDev_PushButtons_Init(input_dev_t *dev, input_dev_callback_t callback)
{
    ...
    blockingTimer = xTimerCreate(NULL, pdMS_TO_TICKS(BLOCKING_TIME_MS), pdFALSE, (void *)0, _BlockingTimerCallback);
