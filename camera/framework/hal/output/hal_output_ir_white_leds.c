...
#ifdef ENABLE_OUTPUT_DEV_IrWhiteLeds
...
static hal_output_status_t HAL_OutputDev_IrWhiteLeds_Init(output_dev_t *dev, output_dev_callback_t callback);
...
hal_output_status_t HAL_OutputDev_IrWhiteLeds_Init(output_dev_t *dev, output_dev_callback_t callback)
{
...
{
    int error = 0;
    LOGD("HAL_OutputDev_IrWhiteLeds_Register");
    error = FWK_OutputManager_DeviceRegister(&s_OutputDev_IrWhiteLeds);
    return error;
}
#endif /* ENABLE_OUTPUT_DEV_IrWhiteLeds */
