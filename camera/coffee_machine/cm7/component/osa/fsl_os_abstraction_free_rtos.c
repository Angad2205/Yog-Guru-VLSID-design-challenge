 ...
 * In this case we allocate it from OCRAM instead of SDRAM. */
extern uint8_t g_vitAfeInitializing;
extern uint32_t g_vitAfeMemoryUsed;
extern int8_t g_vitAfeMemory[];
...
#ifdef ENABLE_VIT_ASR
    if (g_vitAfeInitializing)
    {
        p = &g_vitAfeMemory[g_vitAfeMemoryUsed];
        g_vitAfeMemoryUsed += length;
    }
    else
#endif /* ENABLE_VIT_ASR */
    {
        p = (void *)pvPortMalloc(length);
        if (NULL != p)
        {
