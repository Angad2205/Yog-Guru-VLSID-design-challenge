...
int32_t SLN_AUTH_Generate_Hash(uint8_t *msg, size_t msglen, uint8_t *hash, uint32_t hashSize, mbedtls_md_type_t mdType)
{
    ...
    if (SLN_AUTH_OK == ret)
    {
        ...
        flash_page = (char *)pvPortMalloc(FLASH_PAGE_SIZE);
...
int32_t SLN_AUTH_Verify_Signature(uint8_t *vfPem, uint8_t *msg, size_t msglen, uint8_t *msgsig)
{
    ...
    if (SLN_AUTH_OK == ret)
    {
        ...
        ret = SLN_AUTH_Generate_Hash(msg, msglen, hash, mdInfo->size, mdType);
#else
        ret = SLN_AUTH_Generate_HashFast(msg + FLEXSPI_AMBA_BASE, msglen, hash, mdInfo->size, mdType);
