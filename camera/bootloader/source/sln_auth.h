 ...
 * If the message is in FLASH, this function leverages the XIP mechanism so data will not be hard copied inside internal local memory.
 * Be sure that the address of the message is available at all time.
 * Do not use it when reading from remapped address space!!
 *
 ...
 * @param mdType Type of message digest operation to do
 * @return SLN_AUTH_NULL_PTR if msg or hash are NULL
 * SLN_AUTH_NO_MEM if the hashSize is less than required hash memory specified by mdType or if failed to allocated memory for READ operation.
 * SLN_AUTH_ERR if the message digest operation failed
 * SLN_AUTH_OK otherwise
 */
